...
# add some noise to Encoder variance to avoid error
# EPSILON = 1e-16
...
class BaseVAE(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        '''encoder_distribution = torch.distributions.normal.Normal( 
            loc=mu, scale=torch.exp(logstd) + EPSILON
        )
        z_sample = encoder_distribution.rsample() '''
        b = mu.size(0)
        ###########
        r = Variable(logstd.data.new(b, self.latent_dim).normal_())
        z_sample = torch.exp(logstd)*r + mu
    ...
    def _increment_beta(self):
        ...
        if (
            self.global_step > self.beta_warmup 
            and self.global_step % self.beta_step_freq == 0 
        ):
