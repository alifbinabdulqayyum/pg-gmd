...
def retrain_model(model, datamodule, save_dir, version_str, num_epochs, gpu):
    ...
    checkpointer = pl.callbacks.ModelCheckpoint(save_last=True) #, monitor=,)
...
def main_loop(args):
    ...
    # Load data
    datamodule = WeightedJTNNDataset(args, utils.DataWeighter(args), train_frac=1.0)
    datamodule.setup("fit")

    # Load model
    # model = JTVAE(args, datamodule.vocab)
    
    model = JTVAE.load_from_checkpoint(
        args.pretrained_model_file, vocab=datamodule.vocab
    )
    model.beta = model.beta_final  # Override any beta annealing
    ...
    ################################
    # Sample from latent space with unweighted pre-trained model
    sample_x, sample_y = latent_sampling(
        args, model, datamodule, args.samples_per_model, pbar=None
    )
                
    # Append to results dict
    results["sample_points"].append(sample_x)
    results["sample_properties"].append(sample_y)
    results["sample_versions"].append(0)
    ################################
    ...
    with tqdm(
        ...
        for ret_idx in range(num_retrain):
            ...
            if args.samples_per_model > 0:
                ...
                results["sample_points"].append(sample_x)
                results["sample_properties"].append(sample_y)
                results["sample_versions"].append(ret_idx+1)
            ...
            '''
            # datamodule.train_frac = args.n_retrain_epochs
            del datamodule
            datamodule = WeightedJTNNDataset(args, utils.DataWeighter(args), train_frac=args.n_retrain_epochs)
            datamodule.setup("fit")
            x_all_new += x_new
            y_all_new += y_new
            ...
            results["opt_model_version"] += [ret_idx+1]* len(x_new)
